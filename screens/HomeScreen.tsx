...
const PLACEHOLDER_CARD_WIDTH = (SCREEN_WIDTH - HORIZONTAL_MARGIN * 2 - 12 * 2) / 3;
  ...
  return (
    <SafeAreaView className="flex-1 bg-white" edges={['top']}>
      ...
      >
        ...
        >
          <View className="flex-row items-center">
            ...
            <View className="ml-3 flex-1">
              ...
              <Text className="text-lg font-bold text-blue-600">{savedCourseIds.length}개</Text>
            </View>
        ...
        >
          <View className="flex-row items-center">
            ...
            <View className="ml-3 flex-1">
              ...
              <Text className="text-lg font-bold text-blue-600">{publicCourseIds.length}개</Text>
            </View>
            <Text className="text-sm font-medium text-gray-900">전체 보기</Text>
          </View>
        </Pressable>
          ...
          {MOCK_CHAT_ROOMS.length === 0 ? (
            ...
            <View style={{ marginTop: 12, gap: 8 }}>
              {MOCK_CHAT_ROOMS.map((room) => (
                ...
                >
                  <View className="flex-row items-center justify-between">
                    <Text className="font-semibold text-gray-900">{room.name}</Text>
                    {room.unread > 0 && (
        ...
        <View style={{ marginTop: 28 }}>
          <View className="flex-row items-center justify-between">
            <Text className="text-lg font-bold text-gray-900">주변 인기 코스</Text>
            <Pressable
              hitSlop={12}
              onPress={() =>
            ...
            >
              <Text className="text-sm font-medium text-blue-500">자세히 보기</Text>
            </Pressable>
          </View>
          <ScrollView
            horizontal
          ...
          >
            {popularCourses.map((course) => (
              ...
              >
                ...
                <Text className="mt-2 text-xs text-gray-600 text-center" numberOfLines={2}>
                  {course.title}
                </Text>
              </Pressable>
            ))}
