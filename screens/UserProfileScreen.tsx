...
export default function UserProfileScreen(): React.JSX.Element {
  ...
  const [nickname, setNickname] = useState(String(params.nickname ?? "").trim());
  const [avatarUri, setAvatarUri] = useState(String(params.profileImageUrl ?? "").trim());
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#f5f5f9]" edges={["top"]}>
      <View className="flex-row items-center gap-2 border-b border-gray-200 bg-[#f5f5f9] px-4 py-3">
        ...
        <Text className="flex-1 text-lg font-bold text-gray-900">사용자 프로필</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 18, paddingTop: 16, paddingBottom: 28 }}>
        <View className="items-center rounded-2xl border border-gray-200 bg-white px-4 py-6">
          {avatarUri ? (
            <Image source={{ uri: avatarUri }} className="h-24 w-24 rounded-full bg-gray-100" />
          ) : (
          ...
          <Text className="mt-4 text-lg font-bold text-gray-900">{displayName}</Text>
          {userId ? <Text className="mt-1 text-sm text-gray-500">@{userId}</Text> : null}
        </View>
